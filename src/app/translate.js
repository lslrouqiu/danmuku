...
class translate {
  ...
  _move(restart){
    let _this = this;
    let tick = function() {
      if(_this.moveDanmu.length > 100){
        _this._clearDanmu();
      }
      for(let i = 0,len = _this.moveDanmu.length; i < len; i++){
        let danmu = _this.moveDanmu[i],
            tween = danmu.tween;
        if(danmu.disabled){
          continue;
        }
        ...
        if(restart){
          tween.startTime = Date.now() - tween.duriton;
        }
        const ellapsed = Date.now() - tween.startTime;
        const percentComplete = Math.min((ellapsed) / danmu.runtime, 1);
        if (percentComplete < 1) {
          tween.style[_this.transform] = `translateX(-${tween.start + percentComplete * tween.end}px)`;
          tween.duriton = ellapsed;
        } else {
          tween.style[_this.transform] = `translateX(-${tween.start + tween.end}px)`;
          danmu.disabled = true;
        }
      }
      restart = false;
